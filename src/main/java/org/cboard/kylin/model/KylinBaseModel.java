...
public abstract class KylinBaseModel implements Serializable {
  ...
  /**
   * key: table alias , value : full table name <BR/> because one look up table may have multy
   * alias
   */
  ...
  public KylinBaseModel(JSONObject model, Map<String, String> dataSource, Map<String, String> query,
  ...
  public ResponseEntity<String> getTableInfoRest(RestTemplate restTemplate, String table) {
    return restTemplate
        .getForEntity("http://" + serverIp + "/kylin/api/tables/{tableName}", String.class, table);
  }
  ...
  public String geModelSql() {
    ...
    return String.format("%s %s %s", factTable, tableAlias.get(factTable),
