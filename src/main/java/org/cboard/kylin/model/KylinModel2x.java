...
/**
 * Created by zyong on 2017/12/01. columns: Array[alias.column] tableAlias: Map<db.table, alias>,
 * use alias in lookup meta info defined in kylin model XML columnTable: Map<alias.column, db.table>
 * columnType: Map<alias.column, dataType>
 */
public class KylinModel2x extends KylinBaseModel {

  public KylinModel2x(JSONObject model, Map<String, String> dataSource, Map<String, String> query,
  ...
  public ResponseEntity<String> getTableInfoRest(RestTemplate restTemplate, String table) {
    ...
    if ("0".equals(this.kylinVersion[1])) {
      ...
      return restTemplate
          .getForEntity("http://" + serverIp + "/kylin/api/tables/{project}/{tableName}",
  ...
  void initMetaData() throws Exception {
    ...
    model.getJSONArray("dimensions").forEach(e -> {
      ...
      types.entrySet()
          .forEach(entry -> columnType.put(tblAlias + "." + entry.getKey(), entry.getValue()));
  ...
  public String geModelSql() {
    ...
    return String
        .format("%s %s", getTableWithAliasSuffix(factTable), getJoinSql(tableAlias.get(factTable)));
  }
  ...
  private String getJoinSql(String factAlias) {
    String s = model.getJSONArray("lookups").stream().map(e -> {
      ...
      String[] pk = j.getJSONObject("join").getJSONArray("primary_key").stream()
          .map(p -> p.toString()).toArray(String[]::new);
      String[] fk = j.getJSONObject("join").getJSONArray("foreign_key").stream()
          .map(p -> p.toString()).toArray(String[]::new);
      ...
      return String
          .format("\n %s JOIN %s %s ON %s", type, formatTableName(pTable), j.getString("alias"),
              onStr);
