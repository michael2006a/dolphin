...
import static org.cboard.elasticsearch.query.QueryBuilder.BoolType;
import static org.cboard.elasticsearch.query.QueryBuilder.Entry;
import static org.cboard.elasticsearch.query.QueryBuilder.boolFilter;
import static org.cboard.elasticsearch.query.QueryBuilder.json;
import static org.cboard.elasticsearch.query.QueryBuilder.nullQuery;
import static org.cboard.elasticsearch.query.QueryBuilder.rangeQuery;
import static org.cboard.elasticsearch.query.QueryBuilder.termQuery;
import static org.cboard.elasticsearch.query.QueryBuilder.termsQuery;
...
public class ElasticsearchDataProvider extends DataProvider implements Aggregatable, Initializing {
  ...
  public String[] queryDimVals(String columnName, AggConfig config) throws Exception {
    ...
    String[] filtered = response.getJSONObject("aggregations").getJSONObject(columnName)
        .getJSONArray("buckets").stream()
  ...
  private JSONArray getFilter(AggConfig config) {
    ...
    filters.map(e -> separateNull(e)).map(e -> configComponentToFilter(e)).filter(e -> e != null)
        .forEach(result::add);
    return result;
  }
  ...
  private JSONObject configComponentToFilter(ConfigComponent cc) {
    if (cc instanceof DimensionConfig) {
      ...
      compositeConfig.getConfigComponents().stream().map(e -> separateNull(e))
          .map(e -> configComponentToFilter(e)).forEach(boolArr::add);
  ...
  protected JSONObject post(String url, JSONObject request) throws Exception {
    ...
    if (StringUtils.isEmpty(userName) || StringUtils.isEmpty(password)) {
      httpResponse = Request.Post(url).bodyString(request.toString(), ContentType.APPLICATION_JSON)
  ...
  protected JSONObject get(String url) throws Exception {
    ...
    return JSONObject
        .parseObject(EntityUtils.toString(response.getEntity(), dataSource.get(CHARSET)));
  }
  ...
  private JSONObject getAggregation(DimensionConfig d, AggConfig config) {
    ...
    try {
      ...
      switch (types.get(d.getColumnName())) {
        ...
        default:
          Object missing =
              numericTypes.contains(getTypes().get(d.getColumnName())) ? NULL_NUMBER : NULL_STRING;
      ...
      if (StringUtils.isNotEmpty(d.getCustom())) {
        aggregation = json(d.getColumnName(),
            JSONObject.parseObject(d.getCustom()).get("esBucket"));
      }
    } catch (Exception e) {
      LOG.error("", e);
    }
  ...
  private JSONObject buildDateHistAggregation(String columnName, AggConfig config)
      throws Exception {
    if (config == null) {
  ...
  private JSONObject queryBound(String columnName, AggConfig config) {
    ...
    try {
      JSONObject response = post(getSearchUrl(request), request);
      long maxTs = coalesce(
          response.getJSONObject("aggregations").getJSONObject(maxKey).getLong("value"), 0l);
      long minTs = coalesce(
          response.getJSONObject("aggregations").getJSONObject(minKey).getLong("value"), 0l);
  ...
  protected String getMappingUrl() {
    return String.format("http://%s/%s/_mapping/%s", dataSource.get(SERVERIP), query.get(INDEX),
        query.get(TYPE));
  }

  protected String getSearchUrl(JSONObject request) {
    return String.format("http://%s/%s/%s/_search", dataSource.get(SERVERIP), query.get(INDEX),
        query.get(TYPE));
  }
    ...
    if (types == null) {
      synchronized (key.intern()) {
        ...
        if (types == null) {
          ...
          mapping = mapping.getJSONObject(mapping.keySet().iterator().next())
              .getJSONObject("mappings").getJSONObject(query.get(TYPE));
          types = new HashMap<>();
  ...
  public AggregateResult queryAggData(AggConfig config) throws Exception {
    ...
    JSONObject response = post(getSearchUrl(request), request);
    Stream<DimensionConfig> dimStream = Stream
        .concat(config.getColumns().stream(), config.getRows().stream());
    List<ColumnIndex> dimensionList = dimStream.map(ColumnIndex::fromDimensionConfig)
        .collect(Collectors.toList());
    List<ColumnIndex> valueList = config.getValues().stream().map(ColumnIndex::fromValueConfig)
        .collect(Collectors.toList());
    ...
    Map<String, String> types = getTypes();
    int[] numericIdx = dimensionList.stream()
        .filter(e -> numericTypes.contains(types.get(e.getName())))
  ...
  private void getAggregationResponse(JSONObject object, List<String[]> result,
      List<String> parentKeys, int dimensionLevel, List<ColumnIndex> dimensionList,
    ...
    if (dimensionLevel >= dimensionList.size()) {
      ...
      for (Object _bucket : buckets) {
        ...
        getAggregationResponse((JSONObject) _bucket, result, keys, nextLevel, dimensionList,
            valueList);
      }
    }
  }
  ...
  private String getAggregationName(String aggregationType, String columnName) {
    return Hashing.crc32().newHasher().putString(aggregationType + columnName, Charsets.UTF_8)
        .hash().toString();
  }

  private JSONObject getMetricAggregation(List<ValueConfig> configList,
      Map<String, String> typesCache) {
    ...
    configList.stream().forEach(config -> {
      ...
      if (typesCache.containsKey(config.getColumn())) {
        ...
        aggregation.getJSONObject(aggregationName).getJSONObject(type)
            .put("field", config.getColumn());
      } else {
