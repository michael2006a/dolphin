  ...
  protected ClientAnchor drawContent(XlsProcesserContext context) {
    ...
    for (int r = 0; r < tData.size(); r++) {
      ...
      for (int c = 0; c < tData.getJSONArray(r).size(); c++) {
        ...
        mergeRegion.add(new CellRangeAddress(row.getRowNum(), row.getRowNum(), colStart,
...
//                }
    ...
    for (int row = rowHeader; row >= 0; row--) {
      for (int c = 0; c < columnDataCellIdx.size(); c++) {
        VCell vCell = new VCell(context, mergeRegion,
            context.getBoardSheet().getRow(context.getR1() + row)
                .getCell(columnDataCellIdx.get(c)));
        if (mergeKey != null) {
    ...
    mergeCellHelper(mergeRegion, mergedList);
    mergeRegion.stream()
        .filter(e -> e.getFirstColumn() != e.getLastColumn() || e.getFirstRow() != e.getLastRow())
  ...
  private void mergeCellHelper(List<CellRangeAddress> mergeRegion, List<CellHelper> mergeList) {
    ...
    CellRangeAddress[] toMerged = mergeList.stream().map(e -> e.getMergedCell())
        .collect(Collectors.toList()).toArray(new CellRangeAddress[0]);
