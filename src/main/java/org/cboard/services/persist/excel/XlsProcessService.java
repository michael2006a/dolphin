...
public class XlsProcessService {
  ...
  private XlsProcesserContext dashboardToXls(PersistContext persistContext,
      XlsProcesserContext context) {
    ...
    for (JSONArray rw : widgetRows) {
      for (int i = 0; i < rw.size(); i++) {
        ...
        JSONObject v = persistContext.getData()
            .getJSONObject(widget.getLong("widgetId").toString());
  ...
  private void setColorIndex(HSSFWorkbook wb) {
    ...
    customPalette
        .setColorAtIndex(IndexedColors.GREY_25_PERCENT.index, (byte) 235, (byte) 235, (byte) 235);
  }
