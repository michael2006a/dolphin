...
public class MailService {
  ...
  @Value("${mail.smtp.starttls.required:false}")
  private Boolean mail_smtp_starttls_required;

  @Value("${mail.smtp.ssl.enable:false}")
  private Boolean mail_smtp_ssl_enabled;

  private Function<Object, PersistContext> getPersistBoard(
      List<PersistContext> persistContextList) {
  ...
  public String sendDashboard(DashboardJob job) throws Exception {
    ...
    if (baos != null) {
      ByteArrayDataSource ds = new ByteArrayDataSource(baos.toByteArray(),
    ...
    email.setStartTLSRequired(mail_smtp_starttls_required);
