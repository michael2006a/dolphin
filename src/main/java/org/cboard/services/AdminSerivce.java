...
public class AdminSerivce {
  ...
  public String addUser(String userId, String loginName, String userName, String userPassword) {
    String md5 = Hashing.md5().newHasher().putString(userPassword, Charsets.UTF_8).hash()
        .toString();
  ...
  public List<DashboardBoard> getBoardList(String userId) {
    if (adminUid.equals(userId)) {
      ...
      List<Long> resIdList = resList.stream().filter(e -> RolePermission.isEdit(e.getPermission()))
          .map(e -> e.getResId()).distinct().collect(Collectors.toList());
      return boardDao.getBoardList(userId).stream()
          .filter(e -> resIdList.contains(e.getId()) || e.getUserId().equals(userId))
  ...
  public List<DashboardDataset> getDatasetList(String userId) {
    if (adminUid.equals(userId)) {
      ...
      List<Long> resIdList = resList.stream().filter(e -> RolePermission.isEdit(e.getPermission()))
          .map(e -> e.getResId()).distinct().collect(Collectors.toList());
      return datasetDao.getDatasetList(userId).stream()
          .filter(e -> resIdList.contains(e.getId()) || e.getUserId().equals(userId))
          .collect(Collectors.toList());
    }
  }
  ...
  public List<DashboardWidget> getWidgetList(String userId) {
    if (adminUid.equals(userId)) {
      ...
      List<Long> resIdList = resList.stream().filter(e -> RolePermission.isEdit(e.getPermission()))
          .map(e -> e.getResId()).distinct().collect(Collectors.toList());
      return widgetDao.getWidgetList(userId).stream()
          .filter(e -> resIdList.contains(e.getId()) || e.getUserId().equals(userId))
          .collect(Collectors.toList());
    }
  }
