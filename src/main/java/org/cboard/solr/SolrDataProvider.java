...
public class SolrDataProvider extends DataProvider implements Aggregatable, Initializing {
  ...
  private Function<DimensionConfig, String> filter2SolrCondtion = (config) -> {
    ...
    switch (config.getFilterType()) {
      case "=":
      case "eq":
        return config.getColumnName() + ":(" + IntStream.range(0, config.getValues().size()).boxed()
            .map(i -> config.getValues().get(i)).collect(Collectors.joining("OR", " ", " ")) + ")";
      case "≠":
      case "ne":
        return "-" + config.getColumnName() + ":(" + IntStream.range(0, config.getValues().size())
            .boxed().map(i -> config.getValues().get(i)).collect(Collectors.joining("OR", " ", " "))
            + ")";
      case ">":
      ...
      case "(a,b]":
        if (config.getValues().size() >= 2) {
          return config.getColumnName() + ":{" + config.getValues().get(0) + " TO " + config
              .getValues().get(1) + "]";
        } else {
          return null;
        }
  ...
  private synchronized SolrServerPoolFactory getSolrServerPoolFactory(String solrServers,
  ...
  private String getPoolKey(String solrServers, String collectionName) {
    return Hashing.md5().newHasher().putString(solrServers + "_" + collectionName, Charsets.UTF_8)
        .hash().toString();
  }
  ...
  public AggregateResult queryAggData(AggConfig config) throws Exception {
    //获取统计集合
    QueryResponse response = getQueryResponse(dataSource.get("solrServers"),
        query.get("collection"), getAggQuery(config));
    //获取行列字段
    Supplier<Stream<DimensionConfig>> dimStream = () -> Stream
        .concat(config.getColumns().stream(), config.getRows().stream());
    String dimColsStr = assembleDimColumns(dimStream.get()).replaceAll(" ", "");
    List<ColumnIndex> dimensionList = dimStream.get().map(ColumnIndex::fromDimensionConfig)
    ...
    //解析统计集合
    SimpleOrderedMap obj = (SimpleOrderedMap) ((SimpleOrderedMap) response.getResponse()
        .get("facet_counts")).get("facet_pivot");
    ...
    for (Object e : statList) {
      ...
      for (int x = 0; x < fileds.length; x++) {
        if (x <= fileds.length - config.getValues().size() - 1) {
          ...
          if (x == fileds.length - config.getValues().size() - 1) {
            orderedMap = (SimpleOrderedMap) ((SimpleOrderedMap) orderedMap.get("stats"))
                .get("stats_fields");
          } else {
  ...
  private String assembleDimColumns(Stream<DimensionConfig> columnsStream) {
    ...
    columnsStream.map(g -> g.getColumnName()).distinct().filter(e -> e != null)
        .forEach(columns::add);
  ...
  private SolrQuery getAggQuery(AggConfig config) throws Exception {
    ...
    where.setEmptyValue("");
    filters.map(e -> separateNull(e)).map(e -> configComponentToSql(e)).filter(e -> e != null)
        .forEach(where::add);
  ...
  public String viewAggDataQuery(AggConfig ac) throws Exception {
    //返回查询sql
    return "http://" + dataSource.get("solrServers") + "/solr/" + query.get("collection")
        + "/select?" +
