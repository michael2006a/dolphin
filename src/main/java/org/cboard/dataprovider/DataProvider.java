...
public abstract class DataProvider {
   ...
   */
  public final String[] getDimVals(String columnName, AggConfig config, boolean reload)
      throws Exception {
    String[] dimVals = null;
    evalValueExpression(config);
    if (isDataSourceAggInstance()) {
  ...
  private void evaluator(ConfigComponent e) {
    if (e instanceof DimensionConfig) {
      ...
      dc.setValues(
          dc.getValues().stream().flatMap(v -> getFilterValue(v)).collect(Collectors.toList()));
    }
  ...
  private Stream<String> getFilterValue(String value) {
    ...
    if (value == null || !(value.startsWith("{") && value.endsWith("}"))) {
      ...
      list.add(AviatorEvaluator.compile(value.substring(1, value.length() - 1), true).execute()
          .toString());
    }
    return list.stream();
  }
  ...
  public String getLockKey() {
    ...
    return Hashing.md5().newHasher().putString(dataSourceStr + queryStr, Charsets.UTF_8).hash()
        .toString();
  }
