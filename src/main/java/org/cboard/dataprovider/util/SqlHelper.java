...
public class SqlHelper {
  ...
  private Function<DimensionConfig, String> filter2SqlCondtion = (config) -> {
    ...
    if (NULL_STRING.equals(config.getValues().get(0))) {
      switch (config.getFilterType()) {
        case "=":
        case "≠":
          return config.getColumnName() + ("=".equals(config.getFilterType()) ? " IS NULL"
              : " IS NOT NULL");
      }
    }
  ...
  private String rangeQuery(String fieldName, Object from, Object to, boolean includeLower,
  ...
  private String assembleAggValColumns(Stream<ValueConfig> selectStream) {
    ...
    selectStream.map(vc -> sqlSyntaxHelper.getAggStr(vc)).filter(e -> e != null)
        .forEach(columns::add);
  ...
  private String assembleDimColumns(Stream<DimensionConfig> columnsStream) {
    StringJoiner columns = new StringJoiner(", ", "", " ");
    columns.setEmptyValue("");
    columnsStream.map(g -> sqlSyntaxHelper.getProjectStr(g)).distinct().filter(e -> e != null)
        .forEach(columns::add);
    return columns.toString();
  }
