...
public class AggregateCollector<T> implements Collector<T[], Object[], Double[]> {
  ...
  private AggregateCollector(List<ColumnIndex> valueList) {
    ...
    IntStream.range(0, valueList.size())
        .forEach(i -> collectors.set(i, newCollector(valueList.get(i))));
  }
  ...
  public Supplier<Object[]> supplier() {
    ...
    return () -> {
      ...
      IntStream.range(0, valueList.size())
          .forEach(i -> container[i] = collectors.get(i).supplier().get());
      return container;
    };
  }
  ...
  public BinaryOperator<Object[]> combiner() {
    return (a, b) -> {
      IntStream.range(0, a.length)
          .forEach(i -> a[i] = collectors.get(i).combiner().apply(a[i], b[i]));
      return a;
    };
  }
