...
public class JvmAggregator extends InnerAggregator {
  ...
  public AggregateResult queryAggData(AggConfig config) throws Exception {
    ...
    Stream<ColumnIndex> columns = config.getColumns().stream()
    ...
    String[][] result = new String[grouped.keySet().size()][dimensionList.size() + valuesList
        .size()];
    int i = 0;
    ...
    private boolean checkConfigComponent(ConfigComponent component, String[] row) {
      if (component instanceof DimensionConfig) {
        ...
        if ("AND".equalsIgnoreCase(compositeConfig.getType())) {
          return compositeConfig.getConfigComponents().stream()
              .allMatch(e -> checkConfigComponent(e, row));
        } else if ("OR".equalsIgnoreCase(compositeConfig.getType())) {
          return compositeConfig.getConfigComponents().stream()
              .anyMatch(e -> checkConfigComponent(e, row));
        }
      }
      return false;
    }
