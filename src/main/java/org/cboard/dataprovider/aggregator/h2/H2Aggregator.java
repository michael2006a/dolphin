...
public class H2Aggregator extends InnerAggregator {
  ...
  public String[] queryDimVals(String columnName, AggConfig config) throws Exception {
    ...
    exec = String.format(fsql, surround(columnName, "`"), getTmpTblName(), whereStr,
        surround(columnName, "`"));
    LOG.info(exec);
    try (Connection conn = jdbcDataSource.getConnection();
  ...
  public AggregateResult queryAggData(AggConfig config) throws Exception {
    ...
    try (
      ...
      stat.execute(
          "CREATE ALIAS IF NOT EXISTS f_todouble FOR \"org.cboard.dataprovider.aggregator.h2.Functions.parserString2Double\" ");
    ...
    LOG.info("H2 Database queryAggData using time: {} ms",
  ...
  private boolean isTimeout() {
    ...
    if (createTimeStamp == null || System.currentTimeMillis() - createTimeStamp > (12 * 60 * 60
        * 1000)) {
      return true;
    } else {
      return false;
